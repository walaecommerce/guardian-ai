...
IMPORTANT: If a deterministic check FAILED with severity "critical", your overall score MUST reflect this. Do not override deterministic failures with a passing score.`;
    ...
    // Build product knowledge section from listing context
    let listingContextSection = '';
    if (listingContext && typeof listingContext === 'object') {
      const { deriveProductKnowledge, buildKnowledgeAuditSection } = await import("../_shared/productKnowledge.ts");
      const pk = deriveProductKnowledge(listingContext);
      listingContextSection = buildKnowledgeAuditSection(pk);
    }
